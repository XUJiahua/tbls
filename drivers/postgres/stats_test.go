...
func TestCollectStats(t *testing.T) {
	...
	if findTable(s, "public.stats_basic") == nil {
		t.Skip("stats_basic table not found - run testdata/ddl/postgres_stats.sql first")
	}

	// Test Mode A: direct queries
	t.Run("DirectQueries", func(t *testing.T) {
		// Re-analyze for a clean schema
		s := &schema.Schema{Name: "testdb"}
		if err := p.Analyze(context.Background(), s); err != nil {
			t.Fatal(err)
		}
		...
		basicTable := findTable(s, "public.stats_basic")
		if basicTable == nil {
			t.Fatal("public.stats_basic not found after stats collection")
		}
		...
		// name: varchar(100) NOT NULL, 10000 distinct, all length 10
		...
		// email: varchar(255), 10% null (i%10==0)
		...
		// score: double precision NOT NULL, 0.0..99.9
		...
		// status: varchar(20) NOT NULL, 5 distinct, top-N each 2000
		...
		// category: varchar(50) NOT NULL, 20 distinct
		...
		// updated_at: timestamp, 20% null (i%5==0)
		...
		// payload: jsonb NOT NULL → complex type, should skip deep analysis
		...
		if !payloadCol.Stats.SkippedComplexAnalysis {
			t.Error("payload (jsonb) should have SkippedComplexAnalysis=true")
		}

		// --- stats_numbers assertions ---
		numTable := findTable(s, "public.stats_numbers")
		if numTable == nil {
			t.Fatal("public.stats_numbers not found after stats collection")
		}
		...
		// int_val: integer NOT NULL, -500..499
		...
		// float_val: double precision NOT NULL, 1.5..15000.0
		...
		// nullable_int: integer, 20% null (i%5==0)
		...
		// small_distinct: integer NOT NULL, 10 distinct (0..9), each 1000
	...
	t.Run("PgStats", func(t *testing.T) {
		...
		// Verify table stats
		basicTable := findTable(s, "public.stats_basic")
		if basicTable == nil || basicTable.Stats == nil {
			t.Fatal("public.stats_basic table stats missing in pg_stats mode")
		}
		assertInt64(t, "pg_stats.stats_basic.RowCount", basicTable.Stats.RowCount, 10000)

		// Verify column stats populated (pg_stats values are estimated, so just check presence)
		for _, col := range basicTable.Columns {
			if col.Stats == nil {
				t.Errorf("pg_stats: column %s has no stats", col.Name)
			}
		}

		// status column should have 5 distinct values (pg_stats provides this)
		statusCol := findColumn(basicTable, "status")
		if statusCol != nil && statusCol.Stats != nil {
			if statusCol.Stats.DistinctCount != 5 {
				t.Logf("pg_stats status.DistinctCount: got %d (expected ~5, pg_stats estimates may vary)", statusCol.Stats.DistinctCount)
			}
		}

		numTable := findTable(s, "public.stats_numbers")
		if numTable == nil || numTable.Stats == nil {
			t.Fatal("public.stats_numbers table stats missing in pg_stats mode")
		}
		for _, col := range numTable.Columns {
			if col.Stats == nil {
				t.Errorf("pg_stats: column %s has no stats", col.Name)
			}
		}
	...
	t.Run("DetectDateColumns", func(t *testing.T) {
		s := &schema.Schema{Name: "testdb"}
		if err := p.Analyze(context.Background(), s); err != nil {
			t.Fatal(err)
		}
